...
typetable types[] = {
    ...
    { "datetime",   "DATETIME",         "NOT NULL DEFAULT '2000-01-01T00:00:00Z'",
                                                                "UTCTime" },
...
usage:
    ...
    fprintf(ofile, ") ENGINE=INNODB DEFAULT CHARSET=LATIN1;\n");
    fprintf(ofile, "\n\n");
    fclose(ofile);
    ...
    fprintf(ofile, "updateCampaign :: World m => %s -> %s -> m Bool\n", idtype, ucname);
    fprintf(ofile, "updateCampaign pk (%s {..}) = do\n", ucname);
    ...
    fprintf(ofile, "newCampaign :: World m => %s -> m (Maybe %s)\n", ucname, idtype);
    fprintf(ofile, "newCampaign (%s {..}) = do\n", ucname);
    fprintf(ofile, "    ms <- D.getMasterWriteShard\n");
    fprintf(ofile, "    let upd = D.UpdateRecord D.InsertReplace $ Map.fromList\n");
    for (int i = 1; i != ncols; ++i) {
    ...
    fprintf(ofile, "deleteCampaign :: World m => %s -> m Bool\n", idtype);
    fprintf(ofile, "deleteCampaign pk = do\n");
    fprintf(ofile, "    ms <- D.getMasterWriteShard\n");
